...
"""This script applies the simple keyword technique to a set of texts and 
   produces two kinds of output:
   		...
   		2) a spreadsheet containing the code annotations made by the 
   			simple keyword technique. This also classifies predicted 
   ...
   Modify the values of these variables to customize input and output:
   	...
   	- result_dir: path to the output folder. The coded spreadsheets will 
   		be saved in a folder named 'simplek_[coder]' within result_dir.
	...
	dropped. The script named predict_keyword_roc.py can help choose a 
	good score threshold value using your training/development test dataset.
   	...
   	results/simplek_jane (an output spreadsheet). Configuration file examples
